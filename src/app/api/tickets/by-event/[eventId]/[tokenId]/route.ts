...
// GET /api/tickets/by-event/[eventId]/[tokenId] - Get a single ticket by event ID and tokenId
export async function GET(
    _request: NextRequest,
    { params }: { params: Promise<{ eventId: string; tokenId: string }> }
) {
    try {
        ...
        if (!eventId || !tokenId) {
            return NextResponse.json(
                { error: "Event ID and token ID are required" },
                { status: 400 }
            );
        }
